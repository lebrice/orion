...
from orion.benchmark.task import BenchmarkTask
...
def warm_start_task_correlation_figure(
    ...
    target_task: BenchmarkTask = None,
    source_tasks: List[BenchmarkTask] = None,
    ...
    if source_tasks:
        plot_df["source_task"] = [
            str(source_tasks[task_id]) for task_id in plot_df["task_id"]
        ]
